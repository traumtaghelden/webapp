/*
  # Fix Admin Premium Activation for New System

  1. Updates
    - Replace admin_activate_premium to work with user_profiles.account_status
    - Add function to update account_status directly
    - Add support for the new premium system (account_status_type enum)
...
-- Create new admin_activate_premium that works with account_status
...
BEGIN
  ...
  -- Also update weddings table for backward compatibility
  UPDATE weddings
  SET 
    is_premium = true,
...
-- Add comment
COMMENT ON FUNCTION admin_activate_premium IS 'Manually activate premium for a user (new system compatible)';
COMMENT ON FUNCTION admin_set_account_status IS 'Set any account status for a user (advanced admin function)';
