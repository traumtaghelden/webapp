/*
  # Simplify Budget System (Fixed Version)

  ## Changes
  - Fix history trigger issue
  - Simplify payment_status constraint
  - Archive budget_partner_splits
  - Mark deprecated fields
*/

-- Step 1: Temporarily drop the history trigger
DROP TRIGGER IF EXISTS log_budget_item_changes ON budget_items;

-- Step 2: Simplify payment_status in budget_items
UPDATE budget_items
...
-- Step 3: Migrate estimated_cost to actual_cost where needed
UPDATE budget_items
SET actual_cost = GREATEST(actual_cost, COALESCE(estimated_cost, 0))
WHERE estimated_cost > actual_cost;

-- Step 4: Recreate the history trigger (will work after data migration)
CREATE TRIGGER log_budget_item_changes
  AFTER INSERT OR UPDATE OR DELETE ON budget_items
  FOR EACH ROW
  EXECUTE FUNCTION log_budget_change('item_name');

-- Step 5: Drop and recreate payment_status constraint
...
-- Step 6: Archive budget_partner_splits if table exists
DO $$
BEGIN
...
-- Step 7: Simplify budget_payments status
...
-- Step 8: Drop overdue trigger if exists
...
-- Step 10: Mark deprecated columns
