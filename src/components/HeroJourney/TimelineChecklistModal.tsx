...
interface TimelineChecklistModalProps {
  weddingId: string;
  onClose: () => void;
  onNavigate: () => void;
  onComplete: () => void;
...
const CHECKLIST_ITEMS: Omit<ChecklistItem, 'checked'>[] = [
  { id: 'getting_ready', label: 'Getting Ready Zeit eingeplant' },
  { id: 'ceremony_time', label: 'Trauungszeitpunkt festgelegt' },
  { id: 'reception_planned', label: 'Empfang und Sektempfang geplant' },
  { id: 'photo_session', label: 'Fotoshooting-Zeit reserviert' },
  { id: 'dinner_time', label: 'Dinner-Zeit definiert' },
  { id: 'first_dance', label: 'Eröffnungstanz eingeplant' },
  { id: 'party_time', label: 'Party-Zeitraum festgelegt' },
  { id: 'buffer_times', label: 'Pufferzeiten einkalkuliert' },
];

export default function TimelineChecklistModal({
  weddingId,
  onClose,
  onNavigate,
  onComplete
}: TimelineChecklistModalProps) {
  ...
  const handleComplete = async () => {
    try {
      ...
      showToast('Tagesplan als erledigt markiert!', 'success');
      onComplete();
      onClose();
    } catch (error) {
      ...
      >
        ...
        <div className="relative z-10 flex items-start gap-3 p-4 border-b border-gray-700/50 flex-shrink-0/50">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-3xl font-bold text-white mb-1">Euer Tagesplan</h2>
            <p className="text-sm text-gray-300">Plant den Ablauf eures Hochzeitstags</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="relative z-10 px-6 pb-6">
          <div className="bg-[#d4af37]/10 border border-[#F5B800]/30 rounded-xl p-4">
            <p className="text-sm text-gray-300 leading-relaxed">
              💡 <strong className="text-white">Tipp:</strong> Ein durchdachter Zeitplan gibt allen Beteiligten Orientierung und verhindert Stress am Hochzeitstag. Vergesst nicht, Pufferzeiten einzuplanen!
            </p>
          </div>
        </div>
