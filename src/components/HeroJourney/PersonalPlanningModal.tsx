...
import { Star, X, CheckCircle, ArrowRight } from 'lucide-react';
import { supabase } from '../../lib/supabase';
import { useToast } from '../../contexts/ToastContext';

interface PersonalPlanningModalProps {
  weddingId: string;
  onClose: () => void;
  onNavigate: () => void;
  onComplete: () => void;
...
const CHECKLIST_ITEMS: Omit<ChecklistItem, 'checked'>[] = [
  { id: 'bridal_outfit', label: 'Brautkleid/Outfit ausgesucht' },
  { id: 'partner_outfit', label: 'Anzug/Outfit Partner ausgesucht' },
  { id: 'wedding_rings', label: 'Eheringe ausgewählt' },
  { id: 'shoes', label: 'Schuhe besorgt' },
  { id: 'accessories', label: 'Accessoires organisiert' },
  { id: 'hairstyling', label: 'Hairstyling geplant' },
  { id: 'makeup', label: 'Make-up organisiert' },
  { id: 'vows', label: 'Gelübde geschrieben (optional)' },
  { id: 'special_rituals', label: 'Besondere Rituale überlegt' },
];

export default function PersonalPlanningModal({
  weddingId,
  onClose,
  onNavigate,
  onComplete
}: PersonalPlanningModalProps) {
  ...
  const handleComplete = async () => {
    try {
      ...
      showToast('Persönliche Details als erledigt markiert!', 'success');
      onComplete();
      onClose();
    } catch (error) {
      ...
      >
        ...
        <div className="relative z-10 flex items-start gap-3 p-4 border-b border-gray-700/50 flex-shrink-0/50">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-3xl font-bold text-white mb-1">Persönliche Details</h2>
            <p className="text-sm text-gray-300">Macht es zu eurer einzigartigen Hochzeit</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="relative z-10 px-6 pb-6">
          <div className="bg-[#d4af37]/10 border border-[#F5B800]/30 rounded-xl p-4">
            <p className="text-sm text-gray-300 leading-relaxed">
              💡 <strong className="text-white">Tipp:</strong> Die persönlichen Details machen eure Hochzeit einzigartig. Plant genug Zeit für Outfit-Auswahl und probiert alles rechtzeitig aus!
            </p>
          </div>
        </div>
